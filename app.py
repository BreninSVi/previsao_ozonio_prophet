...
st.caption('''Este projeto utiliza a biblioteca Prophet para prever os níveis
           de ozônio em ug/m3. O modelo
           criado foi treinado com dados até o dia 05/05/2023 e possui um erro
            de previsão (RMSE - Erro Quadrático Médio) igual a 17.43 nos dados
           de teste.
           O usuário pode inserir o número de dias para os quais deseja a
           previsão, e o modelo gerará um gráfico
           interativo contendo as estimativas baseadas em dados históricos de
           concentração de O3.
           Além disso, uma tabela será exibida com os valores estimados para
           cada dia.''')
...
if st.session_state.previsao_feita:
    ...
    tabela_previsao = previsao[['ds', 'yhat']].tail(dias)  # extrai apenas as últimas linhas do dataframe que foram expecificaadas na variável 'dias'
    tabela_previsao.columns = ['Data (Dia/Mês/Ano)', 'O3 (ug/m3)']  # mudando o nome das colunas para quem for ler entenda do que se trata
    tabela_previsao['Data (Dia/Mês/Ano)'] = tabela_previsao['Data (Dia/Mês/Ano)'].dt.strftime('%d-%m-%Y')  # Fazendo a alteração para o nosso formato de data
    tabela_previsao['O3 (ug/m3)'] = tabela_previsao['O3 (ug/m3)'].round(2)
    tabela_previsao.reset_index(drop=True, inplace=True)  # Formatando o index
    ...
    st.download_button(
        label='Baixar tabela como csv', data=csv, file_name='previsao_ozonio.csv', mime='text/csv')
